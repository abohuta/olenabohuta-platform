  ...
  return (
    <Link href={href} className="bg-[var(--cream)] p-8 hover:-translate-y-1 transition-transform no-underline block">
      <div className="flex justify-between items-start mb-4">
        <p className="text-xs tracking-[0.3em] uppercase text-[var(--accent-text)]">{tag}</p>
      ...
      {price && (
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs tracking-widest uppercase text-[var(--accent-text)] border-b border-[var(--accent)]">Детальніше →</span>
        </div>
      )}
    </Link>
  );
