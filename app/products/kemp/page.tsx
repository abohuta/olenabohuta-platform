...
function useReveal() {
  React.useEffect(() => {
    const els = document.querySelectorAll('.kemp-reveal');
...
function useParallax() {
  React.useEffect(() => {
    const bg = document.getElementById('parallax-bg') as HTMLElement;
...
function Carousel() {
  ...
  const cardWidth = typeof window !== 'undefined' ? window.innerWidth < 768 ? 296 : 336 : 336;
  return (
    <div className="relative">
      <div className="overflow-hidden" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        <div className="flex gap-4 transition-transform duration-500" style={{ transform: `translateX(-${current * cardWidth}px)` }}>
          {[...images, ...images].map((src, i) => (
            <div key={i} className="flex-none w-[280px] md:w-[320px] rounded-sm overflow-hidden aspect-[9/16] relative">
              <img src={src} alt={`Відгук ${(i % total) + 1}`} className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
      </div>
...
export default function Kemp() {
  ...
  return (
    <main className="w-full overflow-x-hidden" style={{ background: CREAM }}>
      <style>{`
        .kemp-reveal { opacity: 0; transform: translateY(20px); transition: opacity 0.8s cubic-bezier(0.16,1,0.3,1), transform 0.8s cubic-bezier(0.16,1,0.3,1); }
        .kemp-reveal.visible { opacity: 1; transform: none; }
        .kemp-reveal-delay-1 { transition-delay: 0.1s; }
        .kemp-reveal-delay-2 { transition-delay: 0.2s; }
        .kemp-reveal-delay-3 { transition-delay: 0.3s; }
        .vip-module { transition: all 0.4s cubic-bezier(0.16,1,0.3,1); }
        .vip-module:hover { transform: translateY(-4px); box-shadow: 0 20px 50px rgba(89,2,11,0.25); }
        .plan-card { opacity: 0; transform: translateY(40px) scale(0.97); transition: opacity 0.7s cubic-bezier(0.16,1,0.3,1), transform 0.7s cubic-bezier(0.16,1,0.3,1), box-shadow 0.4s ease; }
        .plan-card.visible { opacity: 1; transform: translateY(0) scale(1); }
        .plan-card.visible.featured-card { transform: translateY(0) scale(1.04); }
        .plan-card.visible:hover { transform: translateY(-8px) scale(1.02); box-shadow: 0 30px 70px rgba(89,2,11,0.25) !important; }
        .base-module { transition: all 0.4s cubic-bezier(0.16,1,0.3,1); }
        .base-module:hover { transform: translateY(-4px); box-shadow: 0 16px 40px rgba(89,2,11,0.12); }
        .kemp-link { position: relative; text-decoration: none; }
        .kemp-link::after { content: ''; position: absolute; bottom: -2px; left: 0; width: 0; height: 1px; background: #59020B; transition: width 0.3s ease; }
        .kemp-link:hover::after { width: 100%; }
        @keyframes heroZoom { from { transform: scale(1); } to { transform: scale(1.06); } }
        .for-who-card { transition: all 0.4s cubic-bezier(0.16,1,0.3,1); box-shadow: 0 4px 20px rgba(89,2,11,0.06); }
        .for-who-card:hover { transform: translateY(-6px); box-shadow: 0 20px 50px rgba(89,2,11,0.14); }
        .plan-card-featured { position: relative; }
        .plan-card-featured::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(184,147,106,0.05) 0%, transparent 60%); pointer-events: none; }
        .plan-btn-gold { transition: all 0.3s cubic-bezier(0.16,1,0.3,1); }
        .plan-btn-gold:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(184,147,106,0.4); }
      `}</style>

      {/* NAVBAR — колір сторінки */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 md:px-16 py-4 flex justify-between items-center backdrop-blur-md" style={{ background: 'rgba(254,248,224,0.95)', borderBottom: '1px solid rgba(89,2,11,0.1)' }}>
        ...
        <div className="hidden md:flex gap-8">
          <a href="/navchannya" className="text-xs tracking-widest uppercase no-underline hover:opacity-60 transition-opacity kemp-link" style={{ color: DARK }}>Навчання</a>
          <a href="/pro-olenku" className="text-xs tracking-widest uppercase no-underline hover:opacity-60 transition-opacity kemp-link" style={{ color: DARK }}>Про автора</a>
          <a href="/zakrytyi-klub" className="text-xs tracking-widest uppercase no-underline hover:opacity-60 transition-opacity kemp-link" style={{ color: DARK }}>Закритий Клуб</a>
          <a href="https://docs.google.com/forms/d/1GTJ6oijlCOO7GfUMlhvGealas2Vo5GI7bh-uaAljfdQ/edit" target="_blank" className="text-xs tracking-widest uppercase no-underline border-b" style={{ color: ACCENT, borderColor: ACCENT }}>Залишити заявку</a>
        </div>
      </nav>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 pt-[70px]" style={{ background: '#59020B', minHeight: '100svh' }}>
        <div className="px-6 md:px-20 py-20 md:py-32 flex flex-col justify-center items-center text-center">
          <a href="/navchannya" className="text-xs tracking-widest uppercase no-underline mb-8 hover:opacity-60 transition-opacity inline-block" style={{ color: 'rgba(254,248,224,0.6)' }}>← Всі навчання</a>
          <p className="text-xs tracking-[0.35em] uppercase mb-6 kemp-reveal" style={{ color: '#F5D5A0' }}>Кемп · Навчання · Особистий бренд</p>
          
          {/* Логотип кемпу */}
          <div className="flex justify-center mb-8">
            <img src="https://i.ibb.co/5gnTQb2H/architect.png" alt="Кемп Архітектор Бренду" style={{ height: '220px', width: 'auto', filter: 'brightness(0) invert(1) sepia(1) saturate(0.5) hue-rotate(5deg)', opacity: 0.9 }} />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 kemp-reveal kemp-reveal-delay-3 justify-center">
            <a href="https://docs.google.com/forms/d/1GTJ6oijlCOO7GfUMlhvGealas2Vo5GI7bh-uaAljfdQ/edit" target="_blank" className="inline-block px-10 py-4 text-xs tracking-widest uppercase no-underline text-center transition-opacity hover:opacity-80" style={{ background: CREAM, color: '#59020B' }}>
  Заповнити анкету
</a>
<a href="#program" className="inline-block px-10 py-4 text-xs tracking-widest uppercase no-underline text-center transition-opacity hover:opacity-80" style={{ border: `1px solid ${CREAM}`, color: CREAM }}>
  Програма
</a>
          </div>
        </div>
        <div className="relative min-h-[60vh] md:min-h-screen overflow-hidden" style={{ minHeight: '60vw' }}>
          <img
              src="https://i.ibb.co/Ps8FK4PZ/hero-olenka.webp"
              alt="Олена Богута"
      ...
      <section className="relative py-20 md:py-32 overflow-hidden" id="results-section">
        ...
        <div className="relative z-10 px-6 md:px-20">
          ...
          <h2 className="text-4xl md:text-5xl font-medium text-center mb-16 leading-tight kemp-reveal" style={{ color: CREAM, fontFamily: 'var(--font-heading)' }}>
      ...
      <section id="program" className="px-6 md:px-20 py-20 md:py-32" style={{ background: CREAM }}>
        ...
        <h2 className="text-4xl md:text-5xl font-medium text-center mb-4 leading-tight kemp-reveal" style={{ color: DARK, fontFamily: 'var(--font-heading)' }}>
          ...
          {[
            ...
            <div key={mod.num} className="p-8 relative kemp-reveal base-module" style={{ background: '#FDF5D5' }}>
              ...
              <ul className="list-none space-y-1">
                {mod.items.map((item) => (
                  <li key={item} className="text-sm" style={{ color: '#7A5A5A' }}>— {item}</li>
                ))}
              </ul>
            </div>
          ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-[2px]">
            {[
              ...
              <div key={mod.num} className="p-8 relative vip-module kemp-reveal" style={{ background: '#59020B', border: `1px solid rgba(254,248,224,0.15)` }}>
                <span className="absolute top-4 right-6 font-medium" style={{ fontFamily: 'var(--font-heading)', fontSize: '4rem', color: '#FEF8E0', opacity: 0.56, lineHeight: 1 }}>{mod.num}</span>
...
<h3 className="text-lg font-medium mb-4" style={{ color: CREAM }}>{mod.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-5xl mx-auto items-stretch">
          {([
            ...
            return (
              ...
              >
                {plan.featured && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-xs tracking-widest uppercase px-5 py-1.5 whitespace-nowrap" style={{ background: '#B8936A', color: '#FEF8E0', letterSpacing: '0.2em' }}>
                    Популярний
                  </span>
                )}
                {plan.vip && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-xs tracking-widest uppercase px-5 py-1.5 whitespace-nowrap" style={{ background: '#59020B', color: '#FEF8E0', letterSpacing: '0.2em' }}>
                    VIP
                  </span>
                )}
                ...
                <ul className="list-none space-y-3 mb-10">
                  ...
                  <a href="https://docs.google.com/forms/d/1GTJ6oijlCOO7GfUMlhvGealas2Vo5GI7bh-uaAljfdQ/edit"
                  target="_blank"
                  className="block text-center py-3.5 text-xs tracking-widest uppercase no-underline plan-btn-gold"
                  style={{
      ...
      <section className="px-6 md:px-20 py-20 md:py-32" style={{ background: CREAM }}>
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div className="relative aspect-[3/4] rounded-sm overflow-hidden kemp-reveal">
            <img src="https://i.ibb.co/v4chftpm/about-olenka.webp" alt="Олена Богута" className="w-full h-full object-cover object-top" />
          ...
          <div className="kemp-reveal">
            ...
            <p className="text-base leading-relaxed mb-6 text-justify" style={{ color: '#5A3A3A' }}>
              Олена допомагає християнам будувати особистий бренд, який відображає їхнє покликання. Її підхід поєднує глибоку віру, стратегічне мислення та практичні інструменти соцмереж.
            </p>
            <p className="text-base leading-relaxed mb-8 text-justify" style={{ color: '#5A3A3A' }}>
              Понад 400 клієнтів та учнів. Засновниця першого в Україні навчання для християн у соцмережах.
            </p>
            <div className="flex gap-4">
              <a href="https://t.me/olenabohuta" target="_blank" className="text-xs tracking-widest uppercase no-underline border-b pb-1 hover:opacity-60 transition-opacity kemp-link" style={{ color: ACCENT, borderColor: ACCENT }}>Telegram</a>
              <a href="https://www.instagram.com/olenka.bohuta" target="_blank" className="text-xs tracking-widest uppercase no-underline border-b pb-1 hover:opacity-60 transition-opacity kemp-link" style={{ color: ACCENT, borderColor: ACCENT }}>Instagram</a>
              <a href="/pro-olenku" className="text-xs tracking-widest uppercase no-underline border-b pb-1 hover:opacity-60 transition-opacity kemp-link" style={{ color: '#8B6F52', borderColor: '#8B6F52' }}>Детальніше →</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 md:px-20 py-20 md:py-32" style={{ background: CREAM }}>
        ...
        <div className="text-center mt-12 kemp-reveal">
          ...
          <a href="https://t.me/olenabohuta" target="_blank" className="inline-block px-10 py-4 text-xs tracking-widest uppercase no-underline hover:opacity-80 transition-opacity" style={{ border: `1px solid ${DARK}`, color: DARK }}>
            Написати в Telegram
          </a>
        </div>
      </section>
      ...
      <section className="px-6 md:px-20 py-20 md:py-32 text-center relative overflow-hidden" style={{ background: DARK }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <div className="flex justify-center mb-6 kemp-reveal">
            <Cross size={28} color="#B8936A" />
          </div>
          ...
          <h2 className="text-4xl md:text-6xl font-medium mb-8 leading-tight kemp-reveal" style={{ color: CREAM, fontFamily: 'var(--font-heading)' }}>
            Готовий(а) стати<br />
            <em className="italic" style={{ color: '#B8936A' }}>Архітектором</em><br />
            свого бренду?
          </h2>
          ...
          <a href="https://docs.google.com/forms/d/1GTJ6oijlCOO7GfUMlhvGealas2Vo5GI7bh-uaAljfdQ/edit" target="_blank" className="inline-block px-12 py-4 text-xs tracking-widest uppercase no-underline hover:opacity-80 transition-opacity kemp-reveal" style={{ background: ACCENT, color: CREAM }}>
