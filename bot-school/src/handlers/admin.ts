...
import { getStats, getAllUsers, addCourse, getAllCourses, toggleCourse } from '../school';
...
export function registerAdminHandlers(bot: Bot) {
  ...
  bot.command('addcourse', async (ctx) => {
    ...
    if (!args.includes('|')) {
      await ctx.reply(
        ...
        '`/addcourse Майстер-клас | Онлайн зустріч | 49 | telegram | -1001234567890`',
        { parse_mode: 'Markdown' }
      );
      return;
    }
  ...
  bot.command('courselist', async (ctx) => {
    ...
    const lines = courses.map((c: any) =>
      `${c.active ? '✅' : '❌'} *ID ${c.id}* — ${c.name} ($${c.price_usd})`
    ).join('\n');

    await ctx.reply(
      `*Всі курси:*\n\n${lines}\n\nВимкнути/увімкнути: /deactivate ID або /activate ID`,
      { parse_mode: 'Markdown' }
    );
  });
